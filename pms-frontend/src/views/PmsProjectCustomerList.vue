<template>
  ...
  <el-breadcrumb separator-class="el-icon-arrow-right">
    ...
    <el-breadcrumb-item :to="'/customerProjects'">定制化</el-breadcrumb-item>
  ...
  methods: {
    ...
    async getProjectsList() {
      if (this.queryInfo.query !== "") {
        ...
        try {
          const {data: res} = await axios.get(`/customerProjects/list?pageNum=${this.queryInfo.pageNum}&pageSize=${this.queryInfo.pageSize}`);
          if (res.code !== 0) {
    ...
    async updateProject() {
      try {
        ...
        const {data: res} = await axios.put(`/customerProjects/update/${this.editProjectForm.id}`, formData, {
          headers: {
            'Content-Type': 'multipart/form-data' // 设置请求的 Content-Type
          }
        }); // 发送创建用户的请求
